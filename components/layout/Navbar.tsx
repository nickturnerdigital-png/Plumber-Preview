...
export function Navbar() {
  ...
  return (
    <>
      ...
      {open && (
        <div
          className="fixed inset-0 z-50 bg-navy/70 backdrop-blur lg:hidden animate-fade-in"
          onClick={() => setOpen(false)}
        >
          <div
            onClick={(e) => e.stopPropagation()}
            className="ml-auto flex h-full w-full max-w-sm flex-col bg-white p-6 animate-slide-in-right"
            ...
            <nav className="mt-10 flex flex-col gap-1">
              {NAV_LINKS.map((l, i) => (
                <Link
                  ...
                  onClick={() => setOpen(false)}
                  className={cn(
                    "block rounded-xl px-4 py-3 text-lg font-semibold transition-colors animate-fade-in-up",
                    l.emphasis
